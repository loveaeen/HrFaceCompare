...
import com.hfits.facerecognition.face.factory.FaceEngineFactory;
...
public class FaceEngineServiceImpl implements FaceEngineService {
    ...
    public void init() {
        ...
        faceEngineComparePool = new GenericObjectPool(new FaceEngineFactory(sdkLibPath, appId, sdkKey,activeKey, compareCfg), comparePoolConfig);//底层库算法对象池
    ...
    public List<FaceInfo> detectFaces(ImageInfo imageInfo) {
        ...
        try {
            ...
            if (faceEngine == null) {
                throw new FaceException(ErrorCodeEnum.FAIL, "获取引擎失败");
            }
    ...
    @Override
    public FaceSimilar compareFace(ImageInfo imageInfo1, ImageInfo imageInfo2) {
    ...
    @Override
    public List<ProcessInfo> processIr(ImageInfo imageInfo, List<FaceInfo> faceInfoList) {
        FaceEngine faceEngine = null;
        try {
            //获取引擎对象
            faceEngine = faceEngineGeneralPool.borrowObject();
            ...
            int errorCode = faceEngine.processIr(imageInfo, faceInfoList, FunctionConfiguration.builder().supportAge(true).supportGender(true).supportLiveness(true).build());
            if (errorCode == 0) {
                List<ProcessInfo> processInfoList = Lists.newLinkedList();
